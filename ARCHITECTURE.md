...
| pmon | client connector · Go | A single static binary each user runs locally. `pmon login` authenticates to the CP (OIDC device auth) and mints a short-lived wire token; the daemon renews it while the server-side session window remains open. It opens one local broker per MySQL or PostgreSQL datasource, where SQL clients use a sticky local password while pmon injects the current token upstream and verifies the proxy's advertised certificate chain and hostname. |
...
- Apps and SQL clients connect through pmon: `pmon login` runs the CP
  ...
  local MySQL or PostgreSQL broker you point any SQL driver at with the sticky
  local password, injecting the token to the proxy over the wire's
  clear-password exchange. When the control plane advertises a certificate
  chain, pmon uses it as the trust pool and verifies the advertised hostname; a
  self-signed leaf works as its own anchor. Without an advertised chain, TLS
  uses the system trust store. A datasource marked as serving wire TLS is
  refused if the proxy declines the upgrade, before the token is sent. A proxy
  configured without wire TLS is brokered in plaintext, which is acceptable only
