	...
	for _, e := range engines {
		t.Run(e.name, func(t *testing.T) {
			cols := []*pb.ColumnSpec{
				...
				columnSpec(e.cat, e.sch, "orders", "id", "BIGINT"),
				columnSpec(e.cat, e.sch, "orders", "amount", "BIGINT"),
			}
			...
			for _, tc := range cases {
				...
				t.Run(tc.sql, func(t *testing.T) {
					facts := analyzeProto(t, &pb.AnalyzeRequest{Sql: tc.sql, EngineConfig: e.ec, Namespace: e.ns, Catalog: cols})
					if !facts.Resolved {
