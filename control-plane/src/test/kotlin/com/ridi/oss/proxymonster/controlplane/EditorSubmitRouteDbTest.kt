...
class EditorSubmitRouteDbTest {
    ...
    @Test
    fun `the editor result re-decision denies a viewer whose shipped rate is spent`() = testApplication {
        val client = wire()
        // A catalog-free statement, so the view releases on its own merits and the rate below is the only
        ...
        try {
            ...
            // 100MB relayed this hour spends the shipped -305 rate; only reachable if the route fed the
            // re-decision an AuditStore.
