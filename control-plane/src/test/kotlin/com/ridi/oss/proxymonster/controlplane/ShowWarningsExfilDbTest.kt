 ...
 * `SELECT CAST(rrn AS UNSIGNED) FROM users ORDER BY (rrn = 'target') DESC LIMIT 1` leaves
 * `Truncated incorrect INTEGER value: '900101-1234567'` there — and `SHOW WARNINGS` reads it back. The proxy
    ...
    fun `SHOW WARNINGS cannot exfiltrate a value the backend leaked into the warning buffer`() {
        ...
        DriverManager.getConnection(fx.targetJdbcUrl, fx.targetUser, fx.targetPassword).use { c ->
            c.createStatement().use { st ->
                st.execute("SELECT CAST(rrn AS UNSIGNED) FROM users ORDER BY (rrn = '$secret') DESC LIMIT 1")
