...
/**
 * The pre-classification `sql.<verb>` datasource actions are gone from the schema. V17 rewrites any policy
...
class SqlVerbActionMigrationDbTest {
    ...
    @Test
    fun `V17 rewrites every sql-verb policy form to its statement category`() {
        val ds = SharedPostgres.hikari(SharedPostgres.freshDatabase("pm_v17"))
        // Migrate to just before V17, then plant operator policies in each legacy form the migration handles:
        // the exact-match scope form (permit and forbid), the single-action in-list, and a multi-action in-list.
        Flyway.configure().dataSource(ds).target(MigrationVersion.fromVersion("16")).load().migrate()
        val planted = mapOf(
        ...
        Flyway.configure().dataSource(ds).load().migrate() // runs V17
        ...
        ds.connection.use { c ->
            c.prepareStatement("SELECT cedar_src FROM policy WHERE name = ?").use { ps ->
                expected.forEach { (name, want) ->
                    ...
                    ps.executeQuery().use { rs ->
                        ...
                        assertEquals(want, rs.getString("cedar_src"), "V17 must rewrite $name to its category")
                    }
                }
            }
            c.createStatement().use { st ->
                st.executeQuery("""SELECT count(*) FROM policy WHERE cedar_src ~ 'Action[[:space:]]*::[[:space:]]*"sql\.(select|insert|update|delete|ddl)"'""").use { rs ->
                    rs.next()
                    assertEquals(0, rs.getInt(1), "no policy may still name a sql.<verb> action after V17")
                }
            }
        }
    }
