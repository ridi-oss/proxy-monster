 ...
 * connect + a data verb (read/write/ddl) but NO `stmt.cat.admin` is denied the admin.* statements at the
 * KIND gate — the class of statement that, before statement classification, slipped through on connect
 * alone because it carried no datasource verb grant (ANALYZE TABLE, SHOW MASTER STATUS, …). The gate is
 * ANDed with the verb category, so `SELECT … INTO OUTFILE` — a `ddl` verb whose kind is `admin.file` —
 * needs admin even from a ddl-granted principal, while a benign schema `SHOW` stays a metadata passthrough.
 * Real MySQL + real Cedar with the shipped schema; asserts the exact kind-gate denyReason so a deny at some
 * other stage would fail the test rather than pass as a false positive.
 */
...
class StatementKindGapDenialDbTest {
    ...
    fun `admin gaps deny at the kind gate for a principal with every benign category but not admin`() {
        ...
        // The admin.maintenance / admin.replication statements deny at the kind gate. Each carries no datasource
        // verb grant, so before classification it fell through the passthrough allow on connect alone; now the
        // kind gate catches it first — and only because the kind is admin, which this principal lacks.
        val gaps = mapOf(
    ...
    @Test
    fun `OUTFILE needs admin_file even for a ddl-granted principal`() {
        // `SELECT … INTO OUTFILE` carries a `ddl` datasource grant, but its kind (select_into_outfile) sits
        // under admin.file. The kind gate is ANDed with the verb, so the ddl grant alone does not authorize
        // server-side file export — the admin.file kind must also be granted.
        val writer = "writer@example.com"
        val d = decide("SELECT 1 INTO OUTFILE '/tmp/pm_probe_out'", writer)
        assertEquals(EnfAction.DENY, d.action, "ddl grant alone must not authorize OUTFILE")
        assertEquals("statement kind 'select_into_outfile' is not permitted", d.denyReason, "OUTFILE denies at the admin.file kind gate")

        // admin.file WITHOUT ddl denies at the VERB loop, not the kind gate — proving the two gates are ANDed,
        // not that admin.file alone suffices. The kind gate clears on admin.file; the ddl verb is still required.
        val fileNoDdl = "filenoddl@example.com"
        val fileRole = fx.policyStore.createRole(RoleInput("file-noddl"))
        fx.policyStore.createAssignment(RoleAssignmentInput(fileNoDdl, fileRole.id))
        fx.cedarPolicyStore.create(
            CedarPolicyInput(
                name = "file-noddl-grant",
                cedarSrc = """permit(principal in Role::"file-noddl", action in [Action::"datasource.connect", Action::"stmt.cat.admin.file"], resource in Datasource::"${fx.datasource.name}");""",
            ),
            updatedBy = "test",
        )
        val noDdl = decide("SELECT 1 INTO OUTFILE '/tmp/pm_probe_out'", fileNoDdl)
        assertEquals(EnfAction.DENY, noDdl.action, "admin.file without ddl must still deny OUTFILE")
        assertTrue(noDdl.denyReason.orEmpty().contains("no stmt.cat.ddl grant"), "denies at the ddl verb loop, proving kind AND verb: ${noDdl.denyReason}")

        // With admin.file (the kind category) AND ddl (the verb), the same statement is authorized — the two
        // gates compose rather than either replacing the other.
        val admin = "admin@example.com"
        val role = fx.policyStore.createRole(RoleInput("admin-op"))
        fx.policyStore.createAssignment(RoleAssignmentInput(admin, role.id))
        fx.cedarPolicyStore.create(
            CedarPolicyInput(
                name = "admin-op-grant",
                cedarSrc = """permit(principal in Role::"admin-op", action in [Action::"datasource.connect", Action::"stmt.cat.admin.file", Action::"stmt.cat.ddl"], resource in Datasource::"${fx.datasource.name}");""",
            ),
            updatedBy = "test",
        )
        assertEquals(EnfAction.ALLOW, decide("SELECT 1 INTO OUTFILE '/tmp/pm_probe_out'", admin).action, "admin.file + ddl authorizes OUTFILE")
    }
