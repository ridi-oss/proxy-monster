...
//
// The 5400 case is `auth config normalizes a mixed-unit absolute cap to minutes`
// (WebSessionRoutesDbTest.kt:124-136) reduced to its pure function: `PM_WEB_SESSION_ABSOLUTE=1h30m`
// must render as 90 MINUTES, not 1.5 hours and not "1 hour 30 minutes".
...
// TestAuthConfigNormalizesAMixedUnitAbsoluteCapToMinutes is WebSessionRoutesDbTest.kt:124-136 at the
// route rather than at the pure function: `PM_WEB_SESSION_ABSOLUTE=1h30m` ⇒ 90 minutes.
...
// (A12 INV-A12-2), recorded at the observable level it can be asserted at TODAY.
//
// ⚠️ HONEST CAVEAT: with A12 unported, [httpapi.Gates.Context] defaults to an EMPTY context, so an
// X-Forwarded-For reaches no decision for a reason that is stronger than the Kotlin's but is NOT the
// Kotlin's reason. The OBSERVABLE outcome is identical — a spoofed header does not make the caller
// an admin — and the case is carried so the route keeps that outcome. It is NOT proof that the
// trusted-edge logic is right; internal/httpapi's trustededge tests are, and A12 owes the end-to-end
// version.
func TestAnUntrustedPeerCannotSpoofRequesterIPAtMePermissions(t *testing.T) {
	az := &recordingAuthorizer{allow: map[authz.AuthzAction]bool{}}
	f := newAuthFixture(t, authTestConfig(false), az) // no trusted proxies, and Gates.Context is nil
