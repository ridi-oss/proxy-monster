...
// In-memory and complete: ConnectionCatalog, RunRequesterIPs, ProxyEventsHub.
// In-memory and REGISTRY-ONLY (their producer services are later increments): RunChannels,
// TableDetailChannels — see registries.go.
//
...
func New(db *store.Db, opts Options) (*ControlPlaneCore, error) {
	...
	return &ControlPlaneCore{
		...
		ProxyEventsHub:       NewProxyEventsHub(),
