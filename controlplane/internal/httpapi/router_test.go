...
//
//	TODO(A1): confirm Ktor's answer to a HEAD on a get-only route during cutover, then decide. If
//	Ktor 405s, the choice is between a HEAD-rejecting middleware here and accepting the widening.
func TestAGETPatternAlsoServesHEAD(t *testing.T) {
	...
	if rec.Code != http.StatusOK || ran != 1 {
		t.Errorf("HEAD /api/tokens = %d with %d handler runs, want 200 and 1 — if this changed, the "+
			"divergence note above is stale and A1's cutover TODO can be closed", rec.Code, ran)
	}
}
