...
// CreatePolicy is `fun createPolicy(input, updatedBy): CedarPolicy`.
...
// CreateRole is `fun createRole(input): Role`, wrapped in `unique("role", input.name)`.
...
// It therefore reaches StatusPages as 500 common.fallback. REPRODUCE: the Kotlin has no 23503 arm,
// and inventing a 404 `notFound("role")` here would be a fix, not a port.
