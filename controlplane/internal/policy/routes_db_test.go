...
func TestAnUnknownRoleIdOnAssignmentCreateReachesTheFallback(t *testing.T) {
	f := newRouteFixture(t)

	rec := f.admin(http.MethodPost, "/api/role-assignments", `{"principal":"alice","roleId":987654}`)

	assertStatus(t, rec, http.StatusInternalServerError, "unknown roleId")
	assertAPIError(t, rec, "common.fallback", "unknown roleId")
	t.Log("REPRODUCED: an FK violation is a 500, not a 404. The Kotlin has no 23503 arm either.")
}
