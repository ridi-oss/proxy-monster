...
//
// ⚠️ SCOPE. The Kotlin suite drives `POST /auth/session/renew` through testApplication. That route
// (`sessionRenewRoutes`) is NOT in this increment — see internal/token's doc.go TODO(A4) — so the
// nine STORE-level cases are ported here against [session.RenewLocked] and its inputs, and the three
// purely-route-level ones are recorded as follows:
//
//	TODO(A4): case 2 (a principal-only JSON body with no bearer is refused — the
//	          unauthenticated-renewal attack), case 3 (a wrong or garbage bearer secret is refused)
//	          and case 4 (a missing Authorization header entirely is refused) assert the route's
//	          THREE distinct 401 codes (INV-A4-35: auth.missing_renewal_token /
//	          common.unauthenticated / auth.session_window_expired). Cases 2 and 3 are already
//	          covered at the store by TestDaemonLookups' GetByRenewalTokenHash assertions — the flaw
//	          they name is "look this up by a caller-supplied principal/handle", and there is no
//	          such lookup to call. What is still unported is the STATUS-CODE distinction.
// ---------------------------------------------------------------------------------------------
