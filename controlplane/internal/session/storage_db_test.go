...
func TestWebIDBySessionKeyReturnsEndedAndExpiredRefs(t *testing.T) {
	...
	idleBefore := f.scalarInt64(`SELECT extract(epoch from idle_expires_at)::bigint FROM principal_session WHERE id = $1`, live)
	...
	if after := f.scalarInt64(`SELECT extract(epoch from idle_expires_at)::bigint FROM principal_session WHERE id = $1`, live); after != idleBefore {
		t.Errorf("the key lookup slid idle_expires_at (%d → %d)", idleBefore, after)
	}
...
func TestSessionTableCarriesItsIndexesAndAPartialUniqueSessionKey(t *testing.T) {
	...
	// The behavioural proof the partial index buys: two rows can both hold a NULL key at once.
