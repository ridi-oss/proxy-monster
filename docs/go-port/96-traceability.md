...
Status as of this document: **497 / 903 Kotlin cases mapped (55.0%)**, 498
accounted (one `KT-DEFER:`), 41 suites at 100%, 44 suites at zero, 0 invalid
markers. Machine-checked by `go test ./internal/tracing/`.
...
The Go control plane had **1,423 test functions** (before this machinery was
added) against the Kotlin's **903 cases**, and `go test ./...` is green. Neither
...
test count of 1423 > 903 is a flattering number that proves nothing.
...
is not more tests — it is a machine-checkable mapping from each of the 903
...
| Piece                                                   | Path                                           |
| ------------------------------------------------------- | ---------------------------------------------- |
| Authoritative case inventory (903 lines)                | `gocp/internal/tracing/kotlin_cases.txt`       |
...
| Tree                                                      | Cases                          |
| --------------------------------------------------------- | ------------------------------ |
| `control-plane/src/test/kotlin/com/ridi/oss/proxymonster` | 847                            |
| `engine/src/test/kotlin/com/ridi/oss/proxymonster`        | 56                             |
| **Total**                                                 | **903** in **121** suite files |

**Correction to earlier notes: 121 suite files, not 116.** There are 126 `.kt`
files under those two trees; 5 are `support/` fixtures with zero cases, leaving
121 with cases. The 903 total was already right.
...
free-form and needs no escaping — necessary because 40 of the 903 case names
...
KT_COVERAGE_REQUIRE_FULL=1 go test ./internal/tracing/   # demand all 903 accounted for
```
