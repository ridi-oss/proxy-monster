...
- **[96-traceability.md](./96-traceability.md)** — the `KT:` marker convention
  ...
  Regenerate the inventory with `go run ./internal/tracing/geninventory.go`;
...
Its corpus grows with each slice. This slice ports no routes, so the corpus
holds only the liveness probe and the harness cannot yet demonstrate equivalence
of anything — the non-vacuity check in `normalize_test.go`, which proves the
normaliser does not collapse a real difference, is the assertion that ships
here.
