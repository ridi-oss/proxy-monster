...
func TestEditorTaskStore(t *testing.T) {
	...
	// KT: EditorTaskStoreDbTest.kt#createEditorTask is born APPROVED as EDITOR with own roles and one child
	...
	// KT: EditorTaskStoreDbTest.kt#createWireTask is born APPROVED as WIRE linked to its decision and has no child
	...
	// KT: EditorTaskStoreDbTest.kt#the born-APPROVED editor task runs the same single-execution status machine
	...
	// KT: EditorTaskStoreDbTest.kt#deleteResultsForTask drops the child but leaves the task, idempotently
	...
	// KT: EditorTaskStoreDbTest.kt#deleteEditorResultsForPrincipal drops only that principal's editor children
	...
	// KT: EditorTaskStoreDbTest.kt#purgeExpiredEditorChildren deletes only expired editor children
	...
	// KT: EditorTaskStoreDbTest.kt#deleteEditorTask cascades the child and is owner + EDITOR scoped
