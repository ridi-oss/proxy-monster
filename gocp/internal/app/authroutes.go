...
// Still owed by A1:
//
//	TODO(A1): the two background timer loops (🔒 INV-A1-5: in loop 1 purgeExpiredEditorChildren()
//	          runs BEFORE purgeExpired()). Every STEP they call now exists — A7's QueryResultStore and
//	          runexec.Service.SweepIdleSessions both landed — so what is owed is the loop itself.
//	          (The SSE /api/tasks/events stream is mounted; see approval.TaskEventsRoute.)
// ---------------------------------------------------------------------------------------------
...
func (rt *authRoutes) debugLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err := httpapi.Receive(r, &login); err != nil {
		// 415 first: ContentNegotiation raises before the handler, so a POST with no body (hence no
		// Content-Type) is 415 — measured (r2-debug-no-body: kotlin=415).
