...
func newAuthServer(t *testing.T, env map[string]string) *authServer {
	...
	// 🔒 The error is FATAL, not skipped: `installMcp`'s capability verification runs inside
	// NewHTTPSurface and a drift between the Cedar action set and the MCP tool catalog must fail the
	// boot (INV-A11-1). A fixture that swallowed it would let every A1 case pass over a control plane
	// that production refuses to start.
	surface, err := NewHTTPSurface(context.Background(), cfg, c, log)
	if err != nil {
		t.Fatalf("NewHTTPSurface: %v", err)
	}
...
// KT: WebSessionRoutesDbTest.kt#debug login resolves through the database and logout ends the row
...
// KT: WebSessionRoutesDbTest.kt#conditional logout ends only the session id observed by the client
...
// KT: WebSessionRoutesDbTest.kt#expired and ended web rows fail closed
...
// KT: WebSessionRoutesDbTest.kt#session observation and ordinary authenticated routes never slide idle while heartbeat does
...
// KT: WebSessionRoutesDbTest.kt#session status and me surface displacement and bind mismatch reasons
...
// KT: WebSessionRoutesDbTest.kt#a pre-cutover principal-roles cookie fails closed to unauthenticated
...
//
// KT: DebugRequesterIpDbTest.kt#a malformed address is refused rather than silently ignored — the route half: the 400 body shape and "a refused login must mint no session"
...
// ---------------------------------------------------------------------------------------------
// 🔒 INV-A1-8 on the DECISION PATH — DebugRequesterIpDbTest.kt:124-154 and :256-…
//
// These two were KT-DEFER while A12's `httpRequesterIp` had no PM_AUTH_DEBUG arm: `/auth/debug`
// STORED `debug_requester_ip` and `/auth/me` read it back, but nothing ever CONSULTED it, so
// "the simulated address replaces the observed peer" and "the stored address is inert once the
// bypass is off" were both unassertable — there was no code path either could be true or false of.
// [httpapi.Gates.HTTPRequesterIP] is that path, so both are reclaimed here.
// ---------------------------------------------------------------------------------------------

// mountRequesterIPProbe is the Kotlin's `serverGet("/test/requester-ip") { call.httpRequesterIp(config) }`
// (DebugRequesterIpDbTest.kt:81) — the substitution observed at the RESOLVER every HTTP decision reads,
// rather than at an inner helper.
//
// authDebug carries the fixture's real config, because the PM_AUTH_DEBUG arm is the subject. The `off`
// variant is the same probe with the bypass disabled, which is what makes the inertness case assertable
// against a row that genuinely holds an address.
func (s *authServer) mountRequesterIPProbe(t *testing.T) {
	t.Helper()
	on := *s.surface.Gates
	offCfg := s.cfg
	offCfg.AuthDebug = false
	...
	probe := func(g httpapi.Gates) http.HandlerFunc {
		return func(w http.ResponseWriter, r *http.Request) {
			if ip := g.HTTPRequesterIP(r); ip != nil {
				_, _ = w.Write([]byte(*ip))
				return
			}
			_, _ = w.Write([]byte("-"))
	...
	s.surface.Router.Mux().HandleFunc("GET /test/requester-ip", probe(on))
	s.surface.Router.Mux().HandleFunc("GET /test/requester-ip-bypass-off", probe(off))
}

func (s *authServer) observedIP(c *http.Client, path string) string {
	s.t.Helper()
	return readBody(s.t, s.do(c, http.MethodGet, path, ""))
}

// debugLoginWithIP is `/auth/debug` carrying a simulated address; nil sends none at all, which is the
// arm that must CLEAR a previously stored one.
func (s *authServer) debugLoginWithIP(c *http.Client, principal string, ip *string) *http.Response {
	s.t.Helper()
	body := `{"principal":"` + principal + `"}`
	if ip != nil {
		body = `{"principal":"` + principal + `","requesterIp":"` + *ip + `"}`
	}
	return s.do(c, http.MethodPost, "/auth/debug", body)
}

// KT: DebugRequesterIpDbTest.kt#a debug login's simulated address replaces the observed peer on the decision path
func TestADebugLoginsSimulatedAddressReplacesTheObservedPeer(t *testing.T) {
	s := newAuthServer(t, nil)
	s.mountRequesterIPProbe(t)
	c := s.client()
	const principal = "sim-ip@example.com"
	const simulated = "100.100.1.10"

	// 🔒 THE BASELINE IS LOAD-BEARING, and it is the Kotlin's own reason for taking it
	// (DebugRequesterIpDbTest.kt:131-132): "without this the substitution assertion below would also
	// pass an implementation that returns the constant". An ordinary login must resolve the OBSERVED
	// socket peer, and that peer must not already be the simulated address.
	if resp := s.debugLoginWithIP(c, principal, nil); resp.StatusCode != http.StatusOK {
		t.Fatalf("baseline login = %d", resp.StatusCode)
	}
	realPeer := s.observedIP(c, "/test/requester-ip")
	if realPeer == simulated {
		t.Fatalf("the socket peer is already %q; the substitution would be unobservable", simulated)
	}
	if realPeer == "-" {
		t.Fatalf("an ordinary login resolved no requester_ip at all; the direct-client arm of "+
			"ResolveHTTPRequesterIP must return the peer (got %q)", realPeer)
	}

	// The substitution itself.
	if resp := s.debugLoginWithIP(c, principal, types.Ptr(simulated)); resp.StatusCode != http.StatusOK {
		t.Fatalf("simulated login = %d", resp.StatusCode)
	}
	if got := s.observedIP(c, "/test/requester-ip"); got != simulated {
		t.Errorf("observed requester_ip = %q, want the simulated %q — the decision path must see the "+
			"stored address, not the socket peer %q", got, simulated, realPeer)
	}

	// 🔒 A LATER LOGIN WITHOUT ONE CLEARS IT rather than inheriting the previous session's. The Kotlin
	// states why: "an address the operator did not ask for is exactly the silently-wrong network this
	// feature exists to make visible."
	if resp := s.debugLoginWithIP(c, principal, nil); resp.StatusCode != http.StatusOK {
		t.Fatalf("clearing login = %d", resp.StatusCode)
	}
	if got := s.observedIP(c, "/test/requester-ip"); got != realPeer {
		t.Errorf("observed requester_ip = %q after a login carrying no address, want the real peer "+
			"%q — a stale simulated address must not survive into the next session", got, realPeer)
	}
}

// KT: DebugRequesterIpDbTest.kt#the stored address is inert once the debug bypass is off
func TestTheStoredSimulatedAddressIsInertOnceTheBypassIsOff(t *testing.T) {
	s := newAuthServer(t, nil)
	s.mountRequesterIPProbe(t)
	c := s.client()
	const principal = "inert-ip@example.com"
	const simulated = "100.100.1.10"

	if resp := s.debugLoginWithIP(c, principal, types.Ptr(simulated)); resp.StatusCode != http.StatusOK {
		t.Fatalf("simulated login = %d", resp.StatusCode)
	}
	// Positive control: the row really does hold the address, so the inertness below is about the
	// bypass FLAG and not about an empty column.
	if got := s.observedIP(c, "/test/requester-ip"); got != simulated {
		t.Fatalf("with the bypass ON the address must be consulted; got %q", got)
	}

	// 🔒 THE SECURITY PROPERTY. With PM_AUTH_DEBUG off, the stored value must never be read: otherwise
	// an address written while the bypass was enabled would keep overriding the observed peer on a
	// control plane that has since been locked down, and a network-gated policy would evaluate against
	// an operator-chosen address forever.
	got := s.observedIP(c, "/test/requester-ip-bypass-off")
	if got == simulated {
		t.Errorf("observed requester_ip = %q with authDebug OFF — the stored simulated address is "+
			"still being consulted; it must be inert", got)
	}
	if got == "-" {
		t.Errorf("observed requester_ip = %q with authDebug OFF, want the real socket peer: the "+
			"bypass arm must be skipped, not the whole resolution", got)
	}
}
