...
//
// goproxy/internal/pb and gocp/internal/pb are two INDEPENDENT protoc-gen-go outputs of the
...
//	  previously from: "github.com/ridi-oss/proxy-monster/gocp/internal/pb"
...
// TestQueryTimeoutMessageMatchesGoproxyVerbatim MACHINE-CHECKS 🔒 INV-A7-31, the cross-language string
// contract, by linking the two producers into one binary and comparing their constants.
//
// # What the contract is
//
// When goproxy's PM_QUERY_TIMEOUT watchdog aborts a statement it sends a RunError whose message is
// EXACTLY `run.QueryTimeoutMessage`. The control plane's collectResponse matches that string to attribute
// the failure as a TIMEOUT — 504 `query.proxy_timeout`, task FAILED — instead of a generic 502
// `query.failed`. And, as goproxy's own comment records, "crucially, to never report a timed-out
// statement as a success: some backends (e.g. MySQL SLEEP) return a row when interrupted, so the
// watchdog's verdict, not the backend's, decides the outcome."
//
// # Why this is a test and not one shared declaration
//
// The Kotlin pair could only be kept in step BY EYE — two constants in two languages with a comment on
// each asking the reader to check. A Go control plane can do better, and this is how far better goes
// today: ONE test that imports both and fails on drift.
//
// 🔴 A direct `const QueryTimeoutMessage = run.QueryTimeoutMessage` in the control plane's production
// code WOULD NOT BOOT. goproxy/run imports goproxy/internal/pb, which registers the file
// "controlplane.proto" in protoregistry.GlobalFiles — and so does controlplane/internal/pb. The protobuf
// runtime's default conflict policy is PANIC, so linking both aborts in init() before main() runs. That
// is the same collision this whole file is build-tagged and env-gated for. The permanent fix is the
// one-directory rename in goproxy described in this file's header; until then the shared constant is a
// checked equality rather than a single declaration, and the check runs in the same CI invocation as the
// rest of the wire suite.
func TestQueryTimeoutMessageMatchesGoproxyVerbatim(t *testing.T) {
	if runexec.QueryTimeoutMessage != run.QueryTimeoutMessage {
		t.Fatalf("the PM_QUERY_TIMEOUT sentinel has DRIFTED between the two sides of the wire:\n"+
			"  control plane (runexec.QueryTimeoutMessage): %q\n"+
			"  proxy         (run.QueryTimeoutMessage):     %q\n"+
			"🔒 INV-A7-31: a timed-out statement is attributed as a timeout ONLY by exact-matching this "+
			"string. With them out of step the control plane reports 502 query.failed for every "+
			"watchdog abort — and, worse, a backend that returns a row when interrupted (MySQL SLEEP) "+
			"would be reported as a SUCCESS.",
			runexec.QueryTimeoutMessage, run.QueryTimeoutMessage)
	}
	// A blank or accidentally-empty constant would make the two "match" while matching nothing on the
	// wire, so the sentinel must also be non-trivial.
	if len(runexec.QueryTimeoutMessage) < 16 || !strings.Contains(runexec.QueryTimeoutMessage, "PM_QUERY_TIMEOUT") {
		t.Errorf("the sentinel %q does not look like the watchdog's message; an empty or truncated "+
			"constant would compare equal on both sides and match nothing on the wire",
			runexec.QueryTimeoutMessage)
	}
}
