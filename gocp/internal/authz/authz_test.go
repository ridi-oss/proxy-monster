...
// KT: AuthzTest.kt#system-admin is allowed on admin actions
...
// KT: AuthzTest.kt#a role other than system-admin is still denied on admin actions
...
// KT: AuthzTest.kt#the audit policies allow own records and grant auditors the whole collection
...
// KT: AuthzTest.kt#an approver may approve someone else's request
...
// KT: AuthzTest.kt#ROLE self-approval is denied even for a system-admin — the self-approval hole stays closed
...
// KT: AuthzTest.kt#a non-admin approving someone else's request is still denied — no ambient permit
...
// KT: AuthzTest.kt#an approval request scoped to a role is matched via 'resource in Role' — Request carries the role as a Cedar parent
...
// KT: AuthzTest.kt#a request scoped to a DIFFERENT role than the policy grants is denied — the Role parent is the request's, not ambient
...
// KT: AuthzTest.kt#task lifecycle and grant revoke actions validate against the bundled schema
...
// KT: AuthzTest.kt#task assume seeds validate and allow only parties or auditor
...
// sibling on Request).
// KT: AuthzTest.kt#approval request marshals executedBy as a User attribute
...
// KT: AuthzTest.kt#retired workflow action ids are rejected by the bundled schema
...
// KT: AuthzTest.kt#an approval request scoped to a datasource is matched via 'resource in Datasource' by NAME
