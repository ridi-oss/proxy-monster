...
// KT: ColumnAuthzTest.kt#an untagged column in the granted table is unmasked
...
// KT: ColumnAuthzTest.kt#a fully-qualified Column EUID grant matches its exact column
...
// KT: ColumnAuthzTest.kt#an identifier containing a key delimiter is denied fail-closed
...
// KT: ColumnAuthzTest.kt#a pii-tagged column in the granted table is masked, not unmasked
...
// KT: ColumnAuthzTest.kt#a column in an ungranted table is denied — deny-by-default, not absent-equals-cleartext
...
// table-scoped
// KT: ColumnAuthzTest.kt#a pii-tagged column in an UNGRANTED table is denied, not masked — the masked grant is table-scoped
...
// KT: ColumnAuthzTest.kt#no roles at all is denied on every column
...
// KT: ColumnAuthzTest.kt#a batch of columns resolves independent verdicts in one call
...
// KT: ColumnAuthzTest.kt#a permit on public users does not cover analytics users with the same table name
...
// KT: ColumnAuthzTest.kt#a permit in one catalog does not cover the same schema and table in another catalog
...
// KT: ColumnAuthzTest.kt#a different datasource with the same qualified table is not covered by the grant
