...
// KT: TagResolutionTest.kt#a tag rule loads though its action is not predefined, and fires when its condition holds
...
// KT: TagResolutionTest.kt#the tag is absent when the raw signal does not match — fail closed
...
// KT: TagResolutionTest.kt#an empty vocabulary short-circuits to no tags
...
// KT: TagResolutionTest.kt#a datasource-scoped tag rule fires only for the named datasource
...
// KT: TagResolutionTest.kt#a derived tag drives a consuming grant end-to-end
...
// KT: TagResolutionTest.kt#an unguarded tag-on-tag rule is rejected at construction — tags absent from the tag-action schema
...
// hole.
// KT: TagResolutionTest.kt#a guarded tag-on-tag rule loads but can never earn a tag — no recursion
...
// effectiveAuthzContext overlay — see 06-query-decision.md §3. A6 HAS since landed, and that half is
// asserted by internal/query's TestEffectiveAuthzContextOverwritesCallerChannelAndTags — the two
// together are the one Kotlin case, which is why both carry the marker.
// KT: TagResolutionTest.kt#effectiveAuthzContext makes channel authoritative and discards caller-supplied tags — the DERIVED-tags half
...
// KT: TagResolutionTest.kt#the dangling-tag lint flags a consumer with no producer and a producer with no consumer
