...
// KT: ConfigGuardTest.kt#bare defaults boot fine (local dev, debug on, dev secret, no oidc)
...
// KT: ConfigGuardTest.kt#debug mode leaves oidc null unless all four required fields are present
...
// KT: ConfigGuardTest.kt#oidc scopes default to openid profile email groups offline_access
...
// KT: ConfigGuardTest.kt#oidc scopes are overridable via PM_OIDC_SCOPES
...
// KT: ConfigGuardTest.kt#debug on plus real oidc config without PM_DEV refuses to start
...
// KT: ConfigGuardTest.kt#debug on plus a real session secret without PM_DEV refuses to start
...
// KT: ConfigGuardTest.kt#debug on plus a production-looking context WITH PM_DEV is allowed
...
// KT: ConfigGuardTest.kt#debug off is always allowed, even with real oidc + session secret
...
// KT: ConfigGuardTest.kt#debug off rejects the public development session secret
...
// KT: ConfigGuardTest.kt#debug off requires complete secure oidc config on the co-hosted callback
...
// KT: ConfigGuardTest.kt#debug off requires secure canonical MCP origins
...
// KT: ConfigGuardTest.kt#session window, web clocks, idp recheck interval, and scim token default sanely
...
// KT: ConfigGuardTest.kt#session window, idp recheck interval, and scim token are overridable
...
// KT: ConfigGuardTest.kt#web idle and slide durations accept units and plain seconds
...
// KT: ConfigGuardTest.kt#web session absolute duration accepts seconds and concatenated units
...
// KT: ConfigGuardTest.kt#web session UX timings accept duration units
...
// KT: ConfigGuardTest.kt#web session slide must be strictly less than idle
...
// KT: ConfigGuardTest.kt#malformed web session durations fail fast
...
// KT: ConfigGuardTest.kt#idp recheck interval must be positive
...
// KT: ConfigGuardTest.kt#PM_TRUSTED_PROXIES parses comma-separated entries, trimmed, with blanks dropped
...
// KT: ConfigGuardTest.kt#PM_QUERY_TIMEOUT defaults overrides and rejects invalid values
...
// KT: ConfigGuardTest.kt#PM_QUERY_TIMEOUT is bounded to the proxy's duration-safe ceiling (no ms overflow)
...
// KT: ConfigGuardTest.kt#the exchange budget outlives the proxy's own statement watchdog
...
// KT: ConfigGuardTest.kt#the run stream outlives the dial and exchange it wraps
...
// A7's two pure TTL formulas are now the SHIPPED symbols, [RunTokenTTLSeconds] and
// [EditorSessionTTLSeconds] in timing.go — this file used to carry local copies, with a TODO(A7)
// saying to delete them the moment the real ones landed, because "a test asserting its own local copy
// of a formula proves nothing about the shipped one". A7 landed (internal/runexec), so they are gone
// and case 25 plus the F26 pin below bind to the real functions.
...
// KT: ConfigGuardTest.kt#run token TTL always outlives the configured query window
func TestRunTokenTTLOutlivesConfiguredQueryWindow(t *testing.T) {
	...
	for _, timeout := range []int64{1, 600, 3600, 36_000, MaxQueryTimeoutSeconds} {
		if !(RunTokenTTLSeconds(timeout) > timeout) {
			t.Errorf("run token TTL must exceed the query window for timeout=%d", timeout)
		}
		if !(EditorSessionTTLSeconds(timeout) > timeout) {
...
func TestF26TimeoutLadderIsNotTotal(t *testing.T) {
	...
	for _, c := range []struct {
		...
		stored := clampTTLSeconds(RunTokenTTLSeconds(cfg.QueryTimeoutSeconds))
		...
		if !(RunTokenTTLSeconds(cfg.QueryTimeoutSeconds) > cfg.QueryTimeoutSeconds) {
