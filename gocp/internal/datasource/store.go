...
// DB exposes the underlying handle, reproducing Kotlin's `internal val dataSource` — the property
// `DatasourceManagementService` reads as `store.dataSource.inTx { … }` (ManagementServices.kt:118).
// Same accessor, same reason, as PolicyStore.DB and identity.UserGroupStore.DB.
func (s *DatasourceStore) DB() *store.Db { return s.db }
