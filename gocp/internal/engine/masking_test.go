...
func TestMasking(t *testing.T) {
	tests := []struct {
		...
		// KT: MaskingTest.kt#last_n reveals only the final four
		{"last_n reveals only final four", stringPointer("900101-1234567"), "LAST_N", stringPointer("**********4567")},
		// KT: MaskingTest.kt#last_n on short values masks entirely — the Kotlin case asserts BOTH "abc" and "1234"; they are the next two rows
		...
		// KT: MaskingTest.kt#format_preserving keeps separators, masks alphanumerics
		{"format preserving", stringPointer("010-1234-5678"), "FORMAT_PRESERVING", stringPointer("***-****-****")},
		// KT: MaskingTest.kt#fixed and null kinds — the Kotlin case's two assertions are the next two rows
		{"fixed", stringPointer("anything"), "FIXED", stringPointer("####")},
		{"null kind", stringPointer("anything"), "NULL", nil},
		// KT: MaskingTest.kt#null input stays null and unknown kind is fully masked — the Kotlin case's two assertions are the next two rows
...
func TestBindMasks(t *testing.T) {
	tests := []struct {
		...
		{
			// KT: BindMasksTest.kt#ordinal binds
			name:        "ordinal binds",
		...
		},
		{
			...
			// KT: BindMasksTest.kt#name is ignored
		...
		{
			// KT: BindMasksTest.kt#out of range ordinal is unbound
		...
		},
		{
			...
			// KT: BindMasksTest.kt#absent ordinal is unbound - never binds to result column 0
		...
		{
			// KT: BindMasksTest.kt#multiple ordinals bind
