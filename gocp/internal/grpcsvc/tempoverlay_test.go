...
//
// KT: GrpcTempOverlayTest.kt#an editor pg_temp column is overlaid under the pg database catalog as a temp
...
// analyzed for it.
//
// KT: GrpcTempOverlayTest.kt#temps are dropped on every non-editor channel — the Kotlin loops WIRE/WORKFLOW_EXECUTOR/WORKFLOW_VIEWER; MCP is the Go port's fourth
...
//
// It doubles as the Kotlin's `a mixed batch keeps only the pg_temp entries`: a batch holding one real
// schema and genuine temps keeps exactly the temps and drops the real one. The all-non-temp batch is
// TestTempOverlayDropsAWholeBatchOfNonTempSchemas.
// KT: GrpcTempOverlayTest.kt#a mixed batch keeps only the pg_temp entries
...
// TestTempOverlayDropsAWholeBatchOfNonTempSchemas is GrpcTempOverlayTest case 2 with the Kotlin's own
// three entries, and the assertion is that the result is EMPTY — not merely "public did not survive".
//
// 🔒 The two system schemas are the half TestTempOverlayDropsANonPgTempSchema cannot state. An overlay
// entry names a schema read UNMASKED and skipping the uncovered-scan gate, so a proxy claiming
// `pg_catalog` or `information_schema` is a temp would read the DATABASE'S OWN METADATA outside every
// classification the catalog holds. Those two are also the schemas a real query most plausibly touches
// by accident, so a filter that special-cased only user schemas would look correct and still be an
// exfiltration primitive.
//
// KT: GrpcTempOverlayTest.kt#a non-pg_temp overlay entry is DROPPED — a proxy cannot mislabel a real table as a temp
func TestTempOverlayDropsAWholeBatchOfNonTempSchemas(t *testing.T) {
	got, err := editorTempOverlay(
		query.ChannelEditor,
		[]*pb.TempColumn{
			tempCol("public", "users", "secret", "text", 1),
			tempCol("pg_catalog", "scratch", "secret", "text", 1),
			tempCol("information_schema", "scratch", "secret", "text", 1),
		},
		enginepb.Engine_POSTGRES, "appdb",
	)
	if err != nil {
	...
	if len(got) != 0 {
		t.Fatalf("overlay = %+v, want EMPTY: only pg_temp* schemas may be overlaid unmasked", got)
	}
}
...
//
// KT: GrpcTempOverlayTest.kt#mysql overlays under the def catalog segment
