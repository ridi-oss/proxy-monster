...
// A12's derivation at the gate, and the anti-spoof invariant with it.
//
// This case used to assert the OPPOSITE — that [Gates.Context] defaulted to an empty AuthzContext
// because httpAuthzContext was unported, so `requester_ip` was absent from every gate decision. It
// existed to make that divergence visible, and named itself after the increment that would flip it.
// A12 has landed, so it is flipped.
//
// 🔒 WHAT IT NOW PINS IS STRONGER THAN "requester_ip is present". The request carries
// `X-Forwarded-For: 203.0.113.9` while its socket peer is httptest's 192.0.2.1, and gateConfig()
// trusts NO proxies. So the header is client-forgeable and must be IGNORED entirely: the resolved
// address has to be the unspoofable TCP peer. Asserting the peer rather than merely non-nil is what
// makes this fail if the resolver ever starts honoring a forwarded header from an untrusted hop —
// which is the exact bug [ResolveHTTPRequesterIP]'s trusted-edge arm exists to prevent, now checked
// through a real gate instead of in isolation.
func TestGateDerivesRequesterIPAndIgnoresAnUntrustedForwardedFor(t *testing.T) {
	...
	g.RequireAdmin(httptest.NewRecorder(), r, authz.ActionAdminIdentity)
	peer, _ := RequestPeer(r)
	if seen.RequesterIP == nil {
		t.Fatalf("requester_ip = nil, want the socket peer %q — the gate must derive it via "+
			"Gates.HTTPAuthzContext when Gates.Context is unset", peer)
	}
	if *seen.RequesterIP != peer {
		t.Errorf("requester_ip = %q, want the socket peer %q. The request's X-Forwarded-For is "+
			"203.0.113.9 and no proxy is trusted, so honoring the header here would be the anti-spoof "+
			"bug: any caller could assert its own source address", *seen.RequesterIP, peer)
	}
	if seen.Channel != nil {
