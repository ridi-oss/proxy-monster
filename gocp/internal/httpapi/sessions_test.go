...
func TestWebSessionCollapsesEveryCookieReadFailureToUnauthenticated(t *testing.T) {
	...
	// 🔴 THE MUTATED BYTE MUST BE ONE WHOSE BITS ARE ALL SIGNIFICANT, AND THE LAST ONE IS NOT.
	//
	// This case used to flip the FINAL character of the cookie to a literal "X", and it forged nothing
	// roughly one run in sixteen. The value is `v1.<base64url(payload)>.<base64url(mac)>` and the MAC
	// is 32 bytes, which base64url-raw encodes as 43 characters — 43×6 = 258 bits for 256 bits of MAC,
	// so the final character carries only FOUR significant bits and two padding bits. Go's decoder is
	// non-strict and ignores those two, so `…U` ("U" = index 20 = 0101_00) and `…X` ("X" = index 23 =
	// 0101_11) decode to the SAME 32 bytes: the MAC still verified, the session still resolved, and the
	// suite failed intermittently with "WebSession resolved …; want no session".
	//
	// Flipping the FIRST character of the MAC instead mutates the top six bits of mac[0], every one of
	// them significant, so the forgery is real on every run. The replacement is chosen against the
	// original rather than hardcoded, so it cannot silently become a no-op again.
	t.Run("a forged cookie", func(t *testing.T) {
		sessions, storage, _ := liveSessions(t)
		cookie := loginCookie(t, sessions, storage, 7)
		cookie.Value = flipOneMACByte(t, cookie.Value)
		r := requestWithIdentity(http.MethodGet, "/probe")
		r.AddCookie(cookie)
		assertNoSession(t, sessions, r)
	})
...
// flipOneMACByte corrupts exactly one FULLY SIGNIFICANT base64 character of a cookie's MAC segment
// and returns the forged value. See the note on the "a forged cookie" case for why "the last
// character" is not such a character.
//
// The MAC is everything after the LAST '.', matching CookieCodec.DecodeRaw's own split. Its first
// character encodes the top six bits of mac[0], all significant, so any different character yields a
// different MAC — and the replacement is derived from the original so the mutation can never be a
// no-op.
func flipOneMACByte(t *testing.T, value string) string {
	t.Helper()
	dot := strings.LastIndexByte(value, '.')
	if dot < 0 || dot+1 >= len(value) {
		t.Fatalf("cookie value has no MAC segment: %q", value)
	}
	replacement := byte('A')
	if value[dot+1] == replacement {
		replacement = 'B'
	}
	return value[:dot+1] + string(replacement) + value[dot+2:]
}
