...
// KT: ResolveRolesTest.kt#resolve unions direct, group, and JIT grant roles
...
// KT: ResolveRolesTest.kt#a revoked grant is excluded from resolve (activeOnly)
...
// KT: ResolveRolesTest.kt#an expired grant is excluded from resolve (activeOnly)
...
// KT: ResolveRolesTest.kt#unknown principal resolves to the empty set (fail-closed)
...
// KT: DeprovisionDbTest.kt#RoleResolver resolve is fail-closed to empty for a deactivated principal, across every role source
...
// KT: DeprovisionDbTest.kt#a principal with no app_user row at all is unaffected by the deactivation gate
...
// KT: ProvisionMergeDbTest.kt#isDeactivated is false for a principal with no app_user row at all — the third row of the table
...
//
// ✅ The blocking half is now asserted: DeprovisionDbTest case 8 — hold the principal's advisory lock
// from a raw connection, commit active=false, and require the in-flight mint to BLOCK and then refuse —
// is ported as internal/token's
// TestMintForActivePrincipalLockedBlocksBehindAConcurrentTeardownAndThenRefuses, which is where
// MintForActivePrincipalLocked lives.
...
// KT: ReadinessDiagnosticDbTest.kt#hasActiveAssignee mirrors resolve across direct group and JIT role paths
