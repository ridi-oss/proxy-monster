...
$$,
     FALSE, 'SYSTEM', 'migration:V18', now()),
...
$$,
     FALSE, 'SYSTEM', 'migration:V18', now()),
-- A session/transaction statement changes connection state, so it is only meaningful on a persistent
-- channel (a wire connection or the editor). On MCP or a workflow — one query per fresh connection — it is
-- denied everywhere, replacing the control-plane's former per-channel deny. Global (no tag scope); a forbid,
-- so it overrides any session permit above.
    (-280, 'preset.session-channel-forbid', 'system:session-channel-forbid',
     $$forbid (
  principal,
  action in [Action::"stmt.cat.session"],
  resource
