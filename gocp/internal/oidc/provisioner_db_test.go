...
// KT: ProvisionMergeDbTest.kt#provisionFromOidc creates a new source=OIDC user and mirrors the claim's groups
...
// KT: ProvisionMergeDbTest.kt#re-provisioning SYNCS group membership to the latest claim (drops removed groups)
...
// KT: ProvisionMergeDbTest.kt#JIT never clobbers a source=SCIM user's fields
...
// KT: ProvisionMergeDbTest.kt#provisionFromOidc reuses an existing group's source, whatever it is
func TestProvision_ExistingGroupKeepsItsOwnSource(t *testing.T) {
	...
	// "whatever it is": the Kotlin case reaches the same branch with a source=LOCAL group, which needs
	// no mapping because its name is not reserved. SYSTEM alone would leave a port that special-cased
	// the reserved source passing while it stamped OIDC over every admin-created group.
	f.seedT.Group("already-local")
	if got := f.groupSource("already-local"); got != "LOCAL" {
		t.Fatalf("precondition: a seeded group is source = %q, want LOCAL", got)
	}
	f.provision("reuse-group@example.com", nil, []string{"already-local"}, GroupMapping{Map: map[string]string{}})
	assertGroups(t, f.groupsOf("reuse-group@example.com"), "already-local")
	if got := f.groupSource("already-local"); got != "LOCAL" {
		t.Errorf("an existing LOCAL group's source = %q after a JIT login, want LOCAL — group "+
			"creation-vs-reuse is the only branch, and reuse must not restamp the source", got)
	}
}
