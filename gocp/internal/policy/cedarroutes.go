...
// non-defaulted field. `{"cedarSrc":"permit(…);"}` with no `name` decodes cleanly to Name:"" here,
// where kotlinx throws MissingFieldException and the Kotlin answers 500 common.fallback. Fixing it
// needs a required-field check encoding/json cannot express, and inventing one would change WHICH
// status a bad body gets on ~120 routes at once, so it is recorded as one divergence rather than
// patched per-DTO.
//
// ⚠️ A11 NARROWED IT. `PolicyManagementService.createPolicy` opens with
// `required("name"); required("cedarSrc")` (ManagementServices.kt:246-247), which A11 §8 restored to
// [PolicyManagement.CreatePolicy]. An absent `name` is now indistinguishable from a BLANK one at
// this layer, so both answer **400 common.field_required** — still not the Kotlin's 500 for the
// absent case, but no longer a 201 that creates a policy named "". The blank case is now exactly
// right. TestCreatePolicyWithNoNameFieldIsAnsweredAsABlankName pins the remaining gap.
//
