...
func (rt *Routes) createAssignment(w http.ResponseWriter, r *http.Request) {
	...
	// 🔒 AssignRoleByID, NOT CreateAssignment. `post("/api/role-assignments")` calls
	// `management.assignRole(input.principal, input.roleId)` (Policies.kt:197) — the VALIDATING overload,
	// which does `required("principal", …)` then resolves the role and 404s when it is absent
	// (ManagementServices.kt:395-399). Wiring this route to the raw store-passthrough instead let a BLANK
	// PRINCIPAL through as a 201-created row and turned an unknown roleId into a 500 FK violation.
	// Measured against the running Kotlin in internal/conformance/differential
	// (r2-create-assignment-blank-principal: kotlin=400 go=201; r2-create-assignment-missing-role:
	// kotlin=404 go=500).
	created, err := rt.management.AssignRoleByID(r.Context(), input.Principal, input.RoleID)
	if err != nil {
		rt.fail(w, r, err)
		return
	}
