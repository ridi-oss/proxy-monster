...
func TestEnforcementPostgresDb(t *testing.T) {
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.masked query returns masked rrn, never cleartext
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.scalar subquery leak is denied and returns no rows
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.correlated subquery oracle over rrn is denied and returns no rows
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.INTERSECT membership oracle over rrn is denied and returns no rows
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.no-FROM query_to_xml data reader is denied and returns no rows
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.no-FROM metadata chatter still runs
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.a principal with zero grants cannot enumerate schema via readonly-meta passthrough
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.non-sensitive query is allowed and returns rows
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.a query on an ungranted table is denied end-to-end — deny-by-default, not cleartext
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.LATERAL correlated leak of rrn is denied and returns no rows
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.recursive CTE anchoring on rrn is denied and returns no rows
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.benign correlated exists is allowed
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.an INSERT without a sql insert grant is denied with a clean reason, not a parse-failure
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.a principal with sql select but no datasource connect is denied first
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.DDL without a sql ddl grant is denied with a clean reason, not a parse-failure
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.CTAS that reads a masked column is denied even with a sql ddl grant
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.CTAS over non-sensitive columns is allowed with a sql ddl grant
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.a no-FROM SELECT INTO cannot bypass the sql ddl gate via readonly-meta passthrough
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.a no-FROM SELECT reading a table via UNION TABLE cannot exfiltrate cleartext rrn
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.an upsert INSERT is denied without sql update, even though sql insert is granted
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.UPDATE and DELETE without their sql grants are denied with a clean reason
	...
	// KT: EnforcementDbTest.kt#EnforcementPostgresDbTest.a provably-total transform of a masked column redacts in full and the rest of the row returns
...
func TestEnforcementMysqlDb(t *testing.T) {
	...
	// KT: EnforcementDbTest.kt#EnforcementMysqlDbTest.masked query returns masked rrn, never cleartext
	// 1. `masked query returns masked rrn, never cleartext`
	t.Run("masked query returns masked rrn, never cleartext", func(t *testing.T) {
		r := run(t, fx, "select id, rrn from users order by id")
		if action(r) != pb.EnfAction_MASK {
	...
	// KT: EnforcementDbTest.kt#EnforcementMysqlDbTest.scalar subquery leak is denied and returns no rows
	// 2. `scalar subquery leak is denied and returns no rows`
	t.Run("scalar subquery leak is denied and returns no rows", func(t *testing.T) {
		r := run(t, fx, "select u.id, (select rrn from users where id = 1) as x from users u")
		assertDenied(t, r, "scalar-subquery rrn leak must be denied")
	...
	// KT: EnforcementDbTest.kt#EnforcementMysqlDbTest.IN subquery oracle is denied
	// 3. `IN subquery oracle is denied`
	t.Run("IN subquery oracle is denied", func(t *testing.T) {
		r := run(t, fx, "select id from users where region in (select rrn from users)")
		assertDenied(t, r, "IN (SELECT rrn ...) oracle must be denied")
	...
	// KT: EnforcementDbTest.kt#EnforcementMysqlDbTest.error-based extraction via extractvalue over a masked column is denied end-to-end
	...
	// KT: EnforcementDbTest.kt#EnforcementMysqlDbTest.SET user-variable from a subquery is denied (session-state exfiltration)
	...
	// KT: EnforcementDbTest.kt#EnforcementMysqlDbTest.a query on an ungranted table is denied end-to-end — deny-by-default, not cleartext
	...
	// KT: EnforcementDbTest.kt#EnforcementMysqlDbTest.an INSERT without a sql insert grant is denied with a clean reason, not a parse-failure
	...
	// KT: EnforcementDbTest.kt#EnforcementMysqlDbTest.a principal with sql select but no datasource connect is denied first
	...
	// KT: EnforcementDbTest.kt#EnforcementMysqlDbTest.CTAS that reads a masked column is denied even with a sql ddl grant
	...
	// KT: EnforcementDbTest.kt#EnforcementMysqlDbTest.a no-FROM SELECT INTO OUTFILE cannot bypass the sql ddl gate (MySQL file-write)
	...
	// KT: EnforcementDbTest.kt#EnforcementMysqlDbTest.a no-FROM SELECT reading a table via UNION TABLE cannot exfiltrate cleartext rrn
	...
	// KT: EnforcementDbTest.kt#EnforcementMysqlDbTest.SELECT INTO after a parenthesized branch cannot bypass the sql ddl gate
