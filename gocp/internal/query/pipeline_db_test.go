...
// client-asserted.
//
// This is TagResolutionTest case 8, WHOLE. The channel-authoritative half can only be stated here —
// effectiveAuthzContext and Channel live in A6, not A2 — and the derived-tags half (a claimed tag is
// never trusted; exactly the DERIVED set survives) is asserted at the end of this test over the
// Kotlin's own tag rule, so nothing about the case is left to a sibling suite. internal/authz's
// TestTagResolution_CallerSuppliedTagsAreDiscarded also carries the marker: it states the derived half
// against A2's own ResolveContextTags, which is where INV-A2-9 lives.
// KT: TagResolutionTest.kt#effectiveAuthzContext makes channel authoritative and discards caller-supplied tags
func TestEffectiveAuthzContextOverwritesCallerChannelAndTags(t *testing.T) {
	...
	// 🔒 THE OTHER ARM OF THE SAME KOTLIN ASSERTION (TagResolutionTest.kt:151-155). With an EMPTY tag
	// vocabulary "the caller's tags were discarded" and "tags are always emptied" are indistinguishable,
	// so the Kotlin also runs the overlay against a policy set that DOES earn a tag and asserts that
	// exactly the DERIVED tag survives. Reproduced here over the Kotlin's own rule (its `cidrTagRule`,
	// TagResolutionTest.kt:47-49) so this test states the whole case rather than half of it.
	tagEngine, err := authz.NewCedarEngineFromSources([]authz.PolicySource{{ID: 1, Src: cidrTagRule}})
	if err != nil {
		t.Fatalf("the tag rule must load: %v", err)
	}
	tagAz := authz.New(tagEngine, nil, authz.RoleSourceFunc(func(string) []string { return nil }))

	// The Kotlin's caller: it asserts channel "editor" AND claims two tags, one of which ("injected") it
	// never earned and one of which ("trusted-network") it could only earn from the raw signal.
	claimedChannel := "editor"
	inRange := "100.100.5.5"
	outOfRange := "10.0.0.1"
	claimant := authz.AuthzContext{
		Channel:     &claimedChannel,
		Tags:        []string{"injected", "trusted-network"},
		RequesterIP: &inRange,
	}

	earned := query.EffectiveAuthzContext(claimant, query.ChannelWire, tagAz,
		"alice", nil, "acme-prod", nil)
	if earned.Channel == nil || *earned.Channel != "wire" {
		t.Errorf("channel = %v, want the SERVER's \"wire\" — the Channel enum must override the caller's",
			earned.Channel)
	}
	if !reflect.DeepEqual(earned.Tags, []string{"trusted-network"}) {
		t.Errorf("tags = %v, want exactly [trusted-network]: only the DERIVED tag survives and \"injected\" is dropped",
			earned.Tags)
	}

	// And with the SAME claimed tags but a signal that earns nothing, the claim buys nothing either —
	// which is what makes the arm above a derivation rather than a pass-through.
	claimant.RequesterIP = &outOfRange
	unearned := query.EffectiveAuthzContext(claimant, query.ChannelWire, tagAz,
		"alice", nil, "acme-prod", nil)
	if len(unearned.Tags) != 0 {
		t.Errorf("tags = %v, want empty: an out-of-range ip earns nothing, and a claimed tag is never trusted",
			unearned.Tags)
	}
}

// cidrTagRule is TagResolutionTest.kt:47-49 verbatim — a tag rule earning "trusted-network" for the
// tailnet CIDR. Its action is NOT predefined in the bundled schema; it loads only because the Cedar
// schema self-augments from the rule (internal/authz's suite pins that separately).
const cidrTagRule = `permit(
