...
// # What is NOT here
//
// Most route halves of A4 — `/auth/device/*` (internal/device), `/auth/oidc/*` (internal/oidc), the
// five `App.kt`-hosted session routes (internal/app) — plus `sweepSessionLiveness` and the OIDC
// discovery/validator pair. This package is the store and the cookie encoding they compose over.
//
// The ONE route it does host is `POST /auth/session/renew` ([RenewRoutes], renewroutes.go), because
// that route is nothing but [RenewLocked] behind a bearer header and hosting it anywhere else would
// mean re-exporting the renewal-hash lookup. It takes its token-mint and deactivation dependencies
// as FUNCTIONS, for the same reason [RenewLocked] is generic: so this package stays free of
// internal/token and internal/identity.
//
// ⚠️ [RenewRoutes] must be mounted EXPLICITLY by the composition root. The Kotlin registers
// `sessionRenewRoutes` from inside `deviceSessionRoutes` (DeviceAuth.kt:359) and internal/device
// deliberately does not, so forgetting it produces no compile error — just a daemon fleet that stops
// renewing twelve hours later.
//
//	TODO(A4): sweepSessionLiveness.
