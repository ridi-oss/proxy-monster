...
func TestPendingOverTheShippedSet(t *testing.T) {
	...
	if len(clean) != len(all) {
		t.Fatalf("clean database: %d pending, want the whole shipped set (%d)", len(clean), len(all))
	}
	if clean[0].script != "V1__identity.sql" || clean[len(clean)-1].script != "V23__result_error_detail.sql" {
		t.Errorf("clean database order = %v, want V1 first and V23 last", versionsOf(clean))
	}
