...
func TestLoadEmbeddedMigrations(t *testing.T) {
	...
	// Versions are consecutive from 1, which is also the numeric-ordering assertion: a lexicographic
	// sort would put V10 between V1 and V2 and this would fail at index 1.
	wantVersions := make([]string, len(all))
	for i := range wantVersions {
		wantVersions[i] = strconv.Itoa(i + 1)
	}
	if len(all) == 0 {
	...
	for i, want := range wantVersions {
		if all[i].version != want {
			t.Errorf("migration %d has version %q, want %q (numeric order)", i, all[i].version, want)
		}
		if len(all[i].body) == 0 {
	...
	if last := all[len(all)-1]; last.script != "V23__result_error_detail.sql" || last.description != "result error detail" {
		t.Errorf("last migration = %q/%q, want V23__result_error_detail.sql/result error detail", last.script, last.description)
	}
}
...
func TestPendingAfter(t *testing.T) {
	...
	if len(pending) != len(all)-9 || pending[0].version != "10" {
		t.Errorf("after version 9: pending = %v, want everything from V10 up", versionsOf(pending))
	}

	newest, _ := parseVersionOrder(all[len(all)-1].version)
	if got := pendingAfter(all, newest); len(got) != 0 {
	...
	if len(pending) != len(all)-5 {
		t.Errorf("after version 5: %d pending, want %d (V6 up)", len(pending), len(all)-5)
	}
