...
//
// The STORE is complete, and so are the four `tokenRoutes` endpoints ([Routes], routes.go). A11's
// MCP token family is not. The web-session half of A4 lives in internal/session.
//
...
// DB is Kotlin's `internal val dataSource` (Tokens.kt:95): the pool exposed so
// `mintForActivePrincipalLocked` CALL SITES can reach it — `store.dataSource.mintForActivePrincipalLocked(…)`
// is literally how both mint routes are written (Tokens.kt:283, :309).
//
// It exists for that one purpose and mirrors *datasource.DatasourceStore.DB(). It is NOT a general
// escape hatch: a caller that wants to run SQL against `proxy_token` should add a method here, so the
// two `IN (…)` kind predicates that carry INV-A4-55/-56 stay in one file.
...
//
// Ownership IS still enforced, so the hole is a cross-KIND one, not a cross-USER one — but now that
// the route exists ([Routes.revoke]) it is worth being exact about WHERE:
//
// ⚠️ CORRECTION, MEASURED. An earlier revision of this comment credited [Store.Revoke]'s
// `AND principal = ?` as the second check. At the DELETE call site that predicate is VACUOUS: the
// route passes `token.principal` — read from the very row it is about to update — so it can never
// fail to match (TestRevokePassesTheTokensOwnerSoAPermittedCrossPrincipalRevokeSucceeds). What
// actually bounds the route to one user is the CEDAR decision on `Token(owner = token.principal)`.
// The SQL predicate is still worth keeping (it bounds any FUTURE caller that passes a principal from
// elsewhere), but it is not what closes the cross-user direction here.
//
