...
//
// It deliberately does NOT start at the full inventory. The port lands as a series of slices, each
// one bringing its own cases across, so the ratchet tracks the series rather than anticipating it: a
// ratchet that is actually green is worth more than an aspiration that is not.
//
...
//
// Set KT_COVERAGE_REQUIRE_FULL=1 to demand the whole inventory instead; that is the switch to flip on
// the day the port claims completeness, and the one CI should flip to prove it.
//
// ⚠️ THE DENOMINATOR MOVES, and that is the one case "never lower it" does not cover. The rule guards
// against a coverage REGRESSION — a Go test deleted without a KT-DEFER in its place. It says nothing
// about the Kotlin suite growing or renaming underneath: regenerate the inventory
// (go run ./internal/tracing/geninventory.go), retarget any marker the checker reports as invalid, and
// set this to the measured "mapped" total. Record WHY in the commit message, not here.
const minMappedCases = 26
