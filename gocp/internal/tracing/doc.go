...
//
// The Go control plane will have more tests than the Kotlin has cases (many Go test functions against
...
//
//   - kotlin_cases.txt — the authoritative inventory: one line per Kotlin @Test case.
//   - geninventory.go — regenerates that file from the Kotlin tree; -check fails if it is stale.
...
//     every *_test.go in the module for markers, and reports the mapped total with the unmapped list.
//
// # The inventory
//
// kotlin_cases.txt is generated from, and must stay in sync with, the Kotlin test tree:
//
//	control-plane/src/test/kotlin/com/ridi/oss/proxymonster  1136 cases
//	engine/src/test/kotlin/com/ridi/oss/proxymonster           56 cases
//	                                                     ------------
//	                                                         1192 cases in 155 suite files
//
// Those totals move as the Kotlin suite does, which is why they are DERIVED rather than maintained:
// regenerate with `go run ./internal/tracing/geninventory.go` and verify in CI with its -check flag.
//
...
//
// Note 155 suite files out of 163 .kt files; the other 8 are support fixtures with zero cases.
// TestInventoryMatchesTheKotlinTree in coverage_test.go re-derives both numbers from the Kotlin tree
// when it is reachable, so the inventory cannot drift silently.
//
// # Case identity
//
