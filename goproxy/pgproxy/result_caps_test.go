		...
		t.Run(fmt.Sprintf("simple=%v", simple), func(t *testing.T) {
			...
			h.fake.decideFn = func(*pb.DecisionRequest) (*pb.WireDecision, error) {
				return wireVerdict(&pb.Verdict{
					Decision: pb.EnfAction_MASK, DecisionId: completionDecisionID, UnmaskedTags: []string{"cap-bytes-50"},
...
func TestResultCapAcrossPortalPages(t *testing.T) {
	...
	seedCapRows(t)
	h.fake.decideFn = func(*pb.DecisionRequest) (*pb.WireDecision, error) {
		return wireVerdict(&pb.Verdict{Decision: pb.EnfAction_ALLOW, DecisionId: completionDecisionID, UnmaskedTags: []string{"cap-rows-250"}}), nil
	}
